...
(function () {
  ...
  const NOW = new Date("2026-09-06T16:10:00+09:00").getTime();
  const MIN = 60 * 1000;
  const WINDOW_MS = 6 * 60 * MIN;
  ...
  const warrants = [
    {
      ...
      lineage: [
        ...
        { t: NOW - 75 * MIN, kind: "연장 +30분", who: "자동 승인", note: "근거: read-only 아님 → 사람 승인 필요. 정팀장 승인" },
  ...
  const warrantById = Object.fromEntries(warrants.map((w) => [w.id, w]));

  // ── 바이너리 카탈로그 — (dev, ino) 가 1급 증거, filename 은 가독성용 ─────
  const BIN = {
  ...
  // ── 이벤트 생성 ─────────────────────────────────────────────────────────
  const events = [];
  const seqByNode = Object.fromEntries(Object.keys(nodes).map((n) => [n, 40000 + between(0, 900)]));
  let pidCounter = 4000;

  function base(node, t, w, extra) {
    const seq = ++seqByNode[node];
    const pid = pidCounter++;
    return Object.assign({
      id: `${node}#${seq}`, node, seq, ts: t,
      warrant: w ? w.id : null, subject: w ? w.subject : null, cgroup: w ? w.cgroup : extra.cgroup,
      pid, tgid: pid, start_time: t - between(1, 900) * 1000, ppid: pid - between(1, 40),
      uid: 1000 + (w ? w.subject - 100 : between(0, 9)), euid: 1000 + (w ? w.subject - 100 : between(0, 9)),
      pid_ns: 4026531836, mnt_ns: 4026531841,
    }, extra);
  }

  function execEv(node, t, w, name, argv, verdict, opts) {
    const b = BIN[name];
    const e = base(node, t, w, Object.assign({
  ...
  function writeEv(node, t, w, path, dev, ino, verdict, flags, opts) {
    if (verdict === "ALLOW") return; // 허용 쓰기는 개별 기록 없음 — 5분 집계 한 건으로 대체 (§11)
    events.push(base(node, t, w, Object.assign({
      hook: "write", verdict, comm: pick(["vim.basic", "bash", "cp", "python3", "tar"]),
      dev, ino, path, parent_ino: ino - between(1, 200), flags,
    }, opts || {})));
  }
  function connectEv(node, t, w, addr, port, verdict, opts) {
    events.push(base(node, t, w, Object.assign({
      hook: "connect", verdict, comm: pick(["curl", "python3", "git", "ssh"]),
      family: addr.startsWith("/") || addr.startsWith("@") ? "AF_UNIX" : "AF_INET", addr, port,
    }, opts || {})));
  }

  // 영장 세션 — 김개발 (W-4821-3F, dryrun): §03 시나리오를 그대로 흘린다
  (function () {
  ...
  const warrantless = [
    { node: "prod-web-02", cgroup: 9350, session: "session-53.scope", uid: 1007, from: "192.168.40.12", first: NOW - 88 * MIN, reason: "직접 접속 — 게이트웨이 기록 없음", severity: "critical" },
    { node: "prod-db-01", cgroup: 5602, session: "session-91.scope", uid: 1002, from: "10.0.3.21", first: NOW - 52 * MIN, reason: "PAM → warrantd 연결 실패 · fail-open 으로 허용 (§17)", severity: "warning" },
    { node: "prod-web-01", cgroup: 8290, session: "session-419.scope", uid: 0, from: "10.0.3.5", first: NOW - 14 * MIN, reason: "root 직접 로그인 · 영장 조회 없음", severity: "critical" },
  ];
  warrantless.forEach((s) => {
    let t = s.first;
    const opts = { cgroup: s.cgroup, uid: s.uid, euid: s.uid };
    ...
    for (let i = 0; i < n; i++) {
      ...
      else if (r < 0.85) execEv(s.node, t, null, pick(["vim", "cp", "python3"]), pick(["vi /etc/cron.d/job", "cp /tmp/.x /usr/local/bin/", "python3 -c \"import socket\""]), "ALLOW", Object.assign({ note: "무영장 — 판정 없음, 귀속만 기록" }, opts));
  ...
  window.SW_DATA = { NOW, WINDOW_MS, MIN, subjects, approvers, nodes, warrants, warrantById, events, warrantless, gaps, HOOKS, BIN };
})();
