...
**S0 완료 · S1 잠정 통과 · S2 핵심 통과.**
BPF LSM이 붙고 돌고, 쓰기 포화 조건에서 오버헤드가 +2.0%, 태그 두 겹이 §04 표대로 동작한다.
...
| `bench/overhead/` | S1 하네스 — 2차 실행 완료(**+2.0% 최악조건**). 티어 E 미측정 |
| `bench/bypass/` | S2 §04 표 = bats 13케이스 + §18 4구멍(skip). **8 통과 · 0 실패** |
| `proto/` `agent/` `pam/` `web/` | 디렉터리 + `README.md` 만. 코드 없음 |
...
## S1에서 실측된 것 (2026-09-01, 커널 6.8.0 · i7-1165G7 8코어)

`bench/overhead` 2차 실행(`out/20260901-003746`), 10회 × 6패스 교차. **티어 E 이전 실행이다.**

- **쓰기 포화 워크로드(`w_untar`, 쓰기 98.6%)에서 D 는 A 대비 +2.0%** (±0.9% 초과, 유의).
  C 는 +1.4%, B 는 노이즈 이하. **한 자릿수 % — S1 판정 통과.**
  여기가 최악 조건이다. 앞문이 아무것도 못 거르는 워크로드다.
- **읽기 지배 워크로드(`w_find`, 쓰기 0.0%, 초당 121,000 open)에서는 분해되지 않는다** (|Δ| < 1.2%).
  마이크로가 이유를 직접 보여준다 — **D 의 p50·p90·p99 가 C 와 같다**(32-63 / 64-127 / 128-255ns).
  cgroup 조회와 맵 2회가 공짜인 게 아니라 **거기까지 가는 호출이 없다.**
  `f_mode & FMODE_WRITE` 앞문 설계의 직접 증거다.
- **훅 1회당 p50: 앞문이 걸러줄 때 32-63ns, 못 거를 때 128-255ns**(`w_untar` 의 D).
  p99.9 는 1-4μs 까지 간다 — 1000건에 1건이다.
- **`w_build` 는 이 실행에서 측정 무효다.** B·D 가 A 보다 35% *빠르게* 나왔다.
  훅은 기계를 빠르게 만들 수 없다 — 고정된 티어 순서가 기계의 주기적 상태 변화와
  정렬된 인공물이다. 슬롯 안에서 3번째 런에 4.1초로 튀고 3.5초로 감쇠한다.
  **고침: 패스마다 티어 순서를 `shuf` 로 섞고, `report.py` 가 "A 보다 빠른 티어"를
  발견하면 그 워크로드를 「측정 무효」로 찍는다.** 조용히 -35.6% 를 보고하지 않는다.
- **태그 조회가 완벽히 맞는다.** `tag_miss=0`, `tag_hit` 이 쓰기 건수와 정확히 일치
  (`w_untar`: 40,237 = 40,237).
- **dev major 는 워크로드의 성질이지 훅의 성질이 아니다.** 여기서는 259(nvme)가 98~99%,
  major 0 은 1% 남짓이다 — **S0 의 유휴 프로파일(major 0 우세)과 정반대다.**
  둘 다 참이고, 그래서 superblock 필터는 여전히 넣지 않는다 (§15).
- **아직 안 잰 것: 티어 E(훅 있고 영장 없음).** 이 실행은 E 추가(`490ab9a`) 전 커밋에서
  돌았다. §13 의 "영장 없는 프로세스는 조회 한 번으로 빠져나간다"는 아직 직접 검증되지
  않았다 — `w_find` 가 간접 증거를 주지만 그건 앞문이 거른 경우고, **무영장 세션은
  앞문을 통과한 뒤 태그 조회에서 빠져나가는 경로**라 다르다.
